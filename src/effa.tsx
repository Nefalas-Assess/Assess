...
const ITP = () => {
  const createRow = () => ({
    ...
    fin: "",
    jours: "",
    indemnite: 30, // Default value for indemnity
    pourcentage: "",
    coefficient: "5/7", // Default value for coefficient
    total: "",
  });
  ...
  const [category, setCategory] = useState("Efforts Accrus");
  ...
  const calculateRow = (row) => {
    ...
    if (debut && fin) {
      ...
      if (!isNaN(debutDate) && !isNaN(finDate)) {
        ...
        // Convert coefficient from string "5/7" to number
        const coeffValue = parseFloat(coefficient.split('/')[0]) / parseFloat(coefficient.split('/')[1]);
        total = (jours * indemnite * (pourcentage / 100) * coeffValue).toFixed(2);
      }
    }
    return { jours, total };
  };
